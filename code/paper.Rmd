...
ggplot(vandal_edits, 
  ...
  geom_point(alpha = 0.2,
    ...
    geom_text_repel(
    data            = vandal_edits %>%  
                      filter(prop_vandal_edits > 0.055),
    segment.size  = 0.2,
    force = 50,
    segment.color = "grey50",
    direction     = "both"
...
enframe(unlist(wh_wiki_talk_pages)) %>% 
  ...
  filter(!word %in% c("article", "page", "talk", "utc", "class", "edit", "project's"))
...
The statistic tf-idf identifies words that are important to a document in a collection of documents; in this case, we’ll see which words are important in one of the talk pages compared to the others.

```{r eval = FALSE}

library(drlib) # devtools::install_github("dgrtwo/drlib")

tidy_talk_tf_idf <- tidy_talk %>%
...
tidy_talk_tf_idf %>%
  ...
  labs(x = NULL, y = "tf-idf",
       title = "Highest tf-idf words in talk pages ",
       subtitle = "")
```
...
bind_rows(
  ...
  ggplot() +
geom_density(aes(as.numeric(edit_span), 
                 fill = source),
                 alpha = 0.3) +
  # scale_x_log10() +
  scale_fill_viridis_d() +  
  theme_minimal(base_size = 12) + 
  xlab("Age of Wikipedia page (days)")
