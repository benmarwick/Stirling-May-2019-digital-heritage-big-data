...
can we identify a cluster of controversial pages? Not really


```{r}

randoms_revision_history_page_details_ip_warring_words_prop_clus <- 
randoms_revision_history_page_details_ip_warring_words_prop %>% 
  left_join(ten_k_random_wp_pages %>% 
  mutate(page = 1:nrow(.))) %>% 
  select(rh_revert_prop, 
         prop_warring_edits,
         rh_diff_size_cv) %>%
   filter_all(all_vars(!is.na(.))) %>% 
  princomp(., scale = TRUE)
...
randoms_revision_history_page_details_ip_time_diffs %>% 
  ...
  mutate(quick_three_edits = time_diff_hours + lag(time_diff_hours) + lag(time_diff_hours, 2) + lag(time_diff_hours, 3)) %>% View
...
randoms_revision_history_page_details_ip_time_diffs %>% 
  bind_cols(rle_tbl2) %>% 
  filter(lths2 == 1) %>% 
  filter(time_diff_hours < 24) %>% View
