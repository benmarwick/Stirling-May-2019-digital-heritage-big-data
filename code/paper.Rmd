...
# density plots
  ...
  facet_wrap( ~ variable, 
              scales = "free") +
  theme_minimal() +
    ggtitle("Basic qualities about attention given to Wikipedia articles")
  
ggsave("figures/wh_wikpedia_pages_attention_given.png")
```
...
revision_history_page_details_ip_addresses_df  %>% 
  ...
  group_by(Site, the_m) %>% 
  tally() %>% 
  filter(n() > 24) # explore this value
...
# subset so we get only sites with > 10 edits/month
...
revision_history_page_details_ip_addresses_df_monthly_edits_anoms <- 
  ...
  unnest(anomalies) %>% 
  group_by(Site) %>% 
  filter(max(anoms) >= 50) # where the anomaly is 50 edits/month or more
